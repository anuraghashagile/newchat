...
import { Send, Loader2, Users, Bot, Zap, RefreshCw, EyeOff, MessageSquare, AlertTriangle, Terminal } from 'lucide-react';
...
export default function App() {
  ...
  // --- 0. FATAL ERROR Screen (DB Setup Missing) ---
  if (activeChat.status === ChatMode.FATAL_ERROR) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-xl w-full bg-slate-900 border border-red-900/50 rounded-2xl p-8 shadow-2xl">
          ...
          <h2 className="text-2xl font-bold text-white mb-4">Final Setup Required</h2>
          <p className="text-slate-400 mb-6 text-sm">
            The database table exists, but the app is blocked from writing to it.
            You must run this command in Supabase to <strong className="text-white">Disable Row Level Security</strong>.
          </p>

          <div className="bg-slate-950 rounded-lg p-4 text-left border border-slate-800 mb-6 relative group">
            <div className="flex items-center gap-2 mb-2 border-b border-slate-800 pb-2">
              <Terminal className="w-4 h-4 text-slate-500" />
              <span className="text-xs font-mono text-slate-500">Supabase SQL Editor</span>
            </div>
            <pre className="text-xs md:text-sm font-mono text-emerald-400 overflow-x-auto whitespace-pre-wrap select-all">
